...
x = tf.compat.v1.placeholder(tf.float32, shape=[None, 60, 80, 8, 3])
...
conv1 = conv_layer(x, shape=[7, 7, 3, 3, 64])
...
def test(sess):
    ...
    # X = test_images.reshape(test_baches, 100, 60, 80, 8, 3)
    # Y = test_labels.reshape(test_baches, 100, 6)



    test_baches = int(len(test_images)/50)
    X = test_images.reshape(test_baches, 50, 60, 80, 8, 3)
    Y = test_labels.reshape(test_baches, 50, 6)
    ...
    # X = test_images.reshape(12, 10, 60, 80, 8, 3)
    # Y = test_labels.reshape(12, 10, 6)
    # acc = np.mean([sess.run(accuracy, feed_dict={x: X[i], y_: Y[i], keep_prob: 1.0}) for i in range(12)])
    # print("Accuracy: {:.5}%".format(acc * 100))
...
with tf.compat.v1.Session() as sess:
    ...
    for i in range(STEPS):
        batch = kth.next_batch(BATCH_SIZE)
